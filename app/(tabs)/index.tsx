          ...
          <View className='flex-row flex-wrap justify-between'>
            { cappuccino == 1 && 
                drink1.slice(0,6).map((item,index) => {
                  return (
                    <TouchableOpacity onPress = {() => router.push({pathname: '/description', params: {image: item.image, name: item.name, price: item.price, ratings: item.ratings, description: item.description}})} key={index} className='w-[48%] h-[300px] bg-[#362c36] px-5 rounded-xl py-5 mt-5'>
